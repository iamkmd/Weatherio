/**
 * @license MIT 
 * @copyright Mansi Kabade 2023 All rights reserved
 * @author Mansi Kabade <mansi.21becem067@gmail.com>
 */
...
searchField.addEventListener("input", function () {
  ...
  if(searchField.value) {
    searchTimeout = setTimeout(() => {
        ...
        searchItem.innerHTML=`                                                                                                                                      <span class="material-icons">location_on</span>                   <div>                                                                                                                                                          <p class="item-title">${name}</p>                                                                                                                              <p class="label-2 item-subtitle">${ state || ""} ${country}</p>                                                                                                  </div>                                                                                                                                                         <a href="#/weather?lat=${lat}&lon=${lon}"  class="item-link has-state" arial-label="${name} weather" data-search-toggler>                                       </a>`;
  ...
  fetchData(url.forecast(lat, lon), function (forecast) {
    ...
    for ( const [index, data] of forecastlist.entries()) {
      ...
      <div class="card card-sm slider-card">
                    ...
                    <img src="${icon}.png" width="48" height="48" alt="${description}" class="weather-icon" loading="lazy" title="${description}">
      ...
      const wind_i = document.createElement("li");
      wind_i.classList.add("slider-item");

      wind_i.innerHTML=`
        <div class="card card-sm slider-card">
                    <p class="body-3">${module.getHours(dateTimeUnix , timezone)}</p>
                 <img src="direction.png" width="48" height="48" alt="" class="weather-icon" loading="lazy" style="transform: rotate(${windDirection - 180}deg)">
        
                            <p class="body-3">${parseInt(module.mps_to_kmh(windSpeed))} km/h</p>
                        </div>
      `;

      hourlySection.querySelector("[data-wind]").appendChild(wind_i); 
    ...
    for(let i = 7, len = forecastlist.length; i < len; i+=8) {
      ...
      li.innerHTML=`
        <div class="icon-wrapper">
            <img src="${icon}.png" width="36" height="36" alt="${description}" class="weather-icon"  title="${description}">
